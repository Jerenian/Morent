...
const Comment = ({navVisible}) => {
    ...
    const [show, setShow] = useState(false)
        const [RecView, setRecView] = useState(false)
        const [PopView, setPopView] = useState(false)
    const ViewChangerRec = () => {
        setRecView(!RecView)
    }
    const ViewChangerPop = () => {
        setPopView(!PopView)
    }
        ...
        const HandleClick = () => {
            setShow(!show)
            setTimeout(() => {
                ReviewsRef.current?.lastElementChild?.scrollIntoView({behavior: 'smooth'})
                console.log(Bigviewport.Rec)
            }, 0);
        }
    ...
    return (
        <div className={classes.Container}>
            ...
            <div  ref = {ReviewsRef} className= {classes.Comment}>
            <Reviews  show={show}/>
            ...
            <div   className={classes.popCars}>
                <div className={classes.Poptitle}>
                    <h1>Popular Car</h1>
                    <button onClick={ViewChangerPop} >View All</button>
                </div>
                    <div data-selector = {PopView} className={classes.carusel}>
                    {CardPopList}
                </div>
            </div>
            <div className={classes.Rectitle}>
                <h1>Recomendation Car</h1>
                <button onClick={ViewChangerRec} >View All</button>
            </div>
                <div data-selector = {RecView} className={classes.recCars} >
                    {CardRecList}
                </div>
             </div>
        </div>
